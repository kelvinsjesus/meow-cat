const formResetPassword = document.getElementById('reset-password');
...
formResetPassword.addEventListener('submit', (e) => {
    ...
    if (newPassword.length < 8) {
        formResponse.innerHTML = `<span class="danger">Crie uma senha com no mínimo 8 caracteres</span>`;
    } else if (newPassword != confirmPassword) {
        formResponse.innerHTML = `<span class="danger">Senhas não coincidem !</span>`;
    } else {
        fetch(`/api/users/password-reset/${token}`, {
            method: 'POST',
            headers: {
            ...
            if(resposta.ok) {
                formResetPassword.innerHTML = `
                    <div id="form-response">
                        <span class="success" style="padding: 1rem; font-size: 20px">Redefinição de senha realizado com sucesso <i class="ri-checkbox-circle-line"></i></span>
                        <a href="login.html" class="to-login">Fazer Login</a>
                    </div
                `;
            } else {
                formResponse.innerHTML = `<span class="danger">Token inválido!</span>`;
            ...
            formResponse.innerHTML = `Desculpe ouve um erro ao redefinir sua senha !`;
        });
    }
})
